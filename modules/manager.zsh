...
.zplugins_manager_init() {
    ...
    if [[ "${ZPLUGINS_USE_AS_MANAGER}" =~ (([yY][eE][sS])|([tT][rR][uU][eE])|1) ]]; then
        .zplugins_log_debug zplugins "setting 'as-manager' to 'yes'"
    ...
    .zplugins_log_debug zplugins "init settings { zsh_loaded_plugins: (${zsh_loaded_plugins[*]}), PMSPEC: (${PMSPEC}), as_manager: ${as_manager} }"
    if [[ (! -v zsh_loaded_plugins && ! -v PMSPEC) || ${as_manager} ]]; then
        .zplugins_log_debug zplugins "as we are the plugin manager then initialize 'zsh_loaded_plugins' and 'PMSPEC'"
...
.zplugins_manager_update() {
    ...
    if .zplugins_as_manager ; then
        .zplugins_log_debug zplugins "as we are the plugin manager then update 'zsh_loaded_plugins'"
